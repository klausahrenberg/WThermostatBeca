...
const byte  MODEL_MCU_BYTE_ECO_MODE[]  			    = {0x05, 0x05, 0x00, 0x00, 0x00, 0x00, 0x67 };
...
public:
  ...
  virtual void printConfigPage(AsyncWebServerRequest* request, Print* page) {
    ...
    page->printf(HTTP_COMBOBOX_ITEM, "1", (getThermostatModel() == 1 ? HTTP_SELECTED : ""), "Heating, Cooling, Ventilation (BAC-002-ALW)");
		...
		page->printf(HTTP_COMBOBOX_ITEM, "6", (getThermostatModel() == 6 ? HTTP_SELECTED : ""), "Floor heating (Touch screen ME102H)");
    	...
    	if (commandLength > -1) {
          ...
          const char* newS;
    			...
    			if (cByte == 0x01 ) {
					...
					} else if ((thModel == MODEL_BHT_002_GBLW) && (cByte == 0x68)) {
